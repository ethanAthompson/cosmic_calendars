...
impl Ceres {
    ...
    pub fn now(&mut self, offset: Ceresian) -> DateTime {
        let now = chrono::Utc::now();
        let now = crate::julian::Julian.get_jd(now.year(), now.month() as i32, now.day() as i32, Ceresian::offset(&offset));

        let date = self.to_date(now);
        let time = Ceresian::now(&offset);
...
impl TimeZone for Ceresian {
    ...
    fn body_host_ratio(&self) -> f64 {
        Ceres.rotational_period() / EARTH_ROTATIONAL_PERIOD
    }
